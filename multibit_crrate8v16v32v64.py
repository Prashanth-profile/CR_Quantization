...
for ind in range(0, min_length, min_l):
    ...
    for mode in range(6):
        ...
        #Unit Step Kernel
            ...
            SDR1_1_norm_1 = noise_removal.savgold_filter(RSSI_16bit_SDR1.raw_samples[ind:ind + min_l],
                                                         win)  # 8-bit Savgol
            SDR2_1_norm_2 = noise_removal.savgold_filter(RSSI_16bit_SDR2.raw_samples[ind:ind + min_l], win)
            #SDR1_1_norm = [float(x) for x in SDR1_1_norm_1]
            #SDR2_1_norm = [float(x) for x in SDR2_1_norm_2]
            #SDR1_1_norm_8 = noise_removal.savgold_filter(RSSI_8bit_SDR1.raw_samples[ind:ind + min_l],
            #                                           win)  # 8-bit Savgol
            #SDR2_1_norm_8 = noise_removal.savgold_filter(RSSI_8bit_SDR2.raw_samples[ind:ind + min_l], win)
...
confidence_interval.plot_confidence_interval(np.array(DCT_16.CR_rate).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "DCT $I = 2$", 'cyan', mark)
