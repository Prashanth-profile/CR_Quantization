...
for ind in range(0, min_length, min_l):
    ...
    for mode in range(6):
        ...
        if mode == 0:
            print("NR stage 1")
            SDR1_1_norm = RSSI_8bit_SDR1.raw_samples[ind:ind + min_l] #8bit
            SDR2_1_norm = RSSI_8bit_SDR2.raw_samples[ind:ind + min_l]
        ...
        #Unit Step Kernel
            ...
            SDR1_1_norm_1=noise_removal.savgold_filter_ali(RSSI_8bit_SDR1.raw_samples[ind:ind + min_l], 9, 5)
        ...
        #Gaussian Kernel
            ...
            SDR1_1_norm_1 = noise_removal.savgold_filter(RSSI_16bit_SDR1.raw_samples[ind:ind + min_l], win-1) #8-bit Savgol
            SDR2_1_norm_2 = noise_removal.savgold_filter(RSSI_16bit_SDR2.raw_samples[ind:ind + min_l], win-1)
        ...
        for k in range(2, maxQuantrange+1):
            ...
            if mode == 0:
                ...
                cap_8 = min(math.log2(min_l), max(sample_entropy, 8))
                No_Filter_8.entropy.append(cap_8)
                ...
                cap_16=min(math.log2(min_l), max(sample_entropy_2, 16))
                No_Filter_16.entropy.append(cap_16)
                entropy = calculate_entropy.calculate_entropy(SDR1_2)
...
confidence_interval.plot_confidence_interval(np.array(No_Filter_16.entropy).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "CR Cap. $\eta=$16", 'magenta', mark_cap)
confidence_interval.plot_confidence_interval(np.array(No_Filter_8.entropy).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "CR Cap. $\eta=$8", 'orange', mark_cap)
