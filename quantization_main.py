...
if Quantization.LOSSY_QUANTIZATION.value == True:
    ...
    for i in range(0, lossy_max_len - remind, min_length):
        SDR1_string = stringify.stringify(lossquantizedbits_SDR1[i:i+min_length])
        SDR2_string = stringify.stringify(lossquantizedbits_SDR2[i:i+min_length])
        ...
        if i==0:
            old_error_dist=error_dist + old_error_dist
        else:
            old_error_dist = np.array(error_dist) + np.array(old_error_dist)

        print("old error", old_error_dist, "of length ", len(old_error_dist))

else:
    for i in range(0, len(list_of_floats_SDR1)-remind, min_length):

        if Quantization.WINDOW_THRESHOLD.value==True:
            threshold_quantized_bits_SDR1 = window_average_threshold_quantization.window_average_meanmedian(list_of_floats_SDR1[i:i+min_length], window_size, Quantization.MEAN_MEDIANBAR.value)
            threshold_quantized_bits_SDR2 = window_average_threshold_quantization.window_average_meanmedian(list_of_floats_SDR2[i:i+min_length], window_size, Quantization.MEAN_MEDIANBAR.value)
        ...
        if Quantization.UNIFORM.value==True:
            uniform_quantized_bytes_SDR1 = uniform_quantization.uniform_quantization_window(list_of_floats_SDR1[i:i+min_length], Quant_Range, window_size)
            uniform_quantized_bytes_SDR2 = uniform_quantization.uniform_quantization_window(list_of_floats_SDR2[i:i+min_length], Quant_Range, window_size)
...
erroranderror_distribution.plot_error_distribution(old_error_dist)
plt.plot(range(len(old_error_dist)), old_error_dist)
plt.xlabel('Bit Position')
plt.ylabel('Number of errors in the bit position')
plt.title('Distribution of Errors with 2048 samples')
plt.show()

plt.hist(old_error_dist, bins=32, density=False, alpha=1, color='b')

# Add a title and labels to the plot
plt.title('Histogram of error distribution')
plt.xlabel('Number of errors per bit position')
