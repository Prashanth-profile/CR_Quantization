...
fig1, ax = plt2.subplots()
plt2.title("Floor difference")
plt2.grid()
#plt2.legend()
plt2.rcParams.update({'font.family': 'Times New Roman', 'font.size': fontsz, })
fig2, axis4 = plt4.subplots()
plt4.title("PDF")
plt4.grid()
#plt4.legend()
fig2, axis5 = plt5.subplots()
plt5.title("Error distribution")
plt5.grid()
#plt5.legend()
...
err_values_clip = []
err_values_clip_gray = []
err_values_USclip = []
err_values_USclip_gray = []
...
for ind in range(0, min_length, min_l):
    ...
    for a in range(4):
        ...
        if (a == 2) or (a==3):
            SDR1_1_norm = noise_removal.window_smoothening(list_of_floats_SDR1[ind:ind + min_l], min_l)
            SDR2_1_norm = noise_removal.window_smoothening(list_of_floats_SDR2[ind:ind + min_l], min_l)
        ...
        for k in range(2, maxQuantrange+1):
            ...
            if a == 0:
                ...
                if (k == 8) & (ind==0):
                    plot_histogram.create_histogram(SDR1_2, 4, axis4, f'{k}-bit', 'blue')
                    plot_histogram.create_histogram(floor_diff, 4, ax, f'{k}-bit', 'blue')
                    print("floor diff", floor_diff)
                    plot_histogram.create_histogram(error_dist, 4, axis5, f'{k}-bit', 'blue')

            elif a == 1:
                err_values_clip_gray.append(num_errors)
                err_values_clip.append(num_errors_norm)
                entropy_clip.append(calculate_entropy.calculate_entropy(SDR1_2))
                CR_rate_clip.append((calculate_entropy.calculate_entropy(SDR1_2))*abs(1-2*(num_errors/(Quant_Range*min_l))))
                if (k == 8) & (ind == 0):
                    plot_histogram.create_histogram(SDR1_2, 4, axis4, f'{k}-bitClip', 'red')
                    plot_histogram.create_histogram(floor_diff, 4, ax, f'{k}-bitClip', 'red')
                    print("floor diff", floor_diff)
                    plot_histogram.create_histogram(error_dist, 4, axis5, f'{k}-bitClip', 'red')
                ...
                if (k == 8) & (ind == 0):
                    plot_histogram.create_histogram(SDR1_2, 4, axis4, f'{k}-bitUS', 'magenta')
                    plot_histogram.create_histogram(floor_diff, 4, ax, f'{k}-bitUS', 'magenta')
                    print("floor diff", floor_diff)
                    plot_histogram.create_histogram(error_dist, 4, axis5, f'{k}-bitUS', 'magenta')

            else:
                err_values_USclip_gray.append(num_errors)
                err_values_USclip.append(num_errors_norm)
                entropy_USclip.append(calculate_entropy.calculate_entropy(SDR1_2))
                CR_rate_USclip.append((calculate_entropy.calculate_entropy(SDR1_2))*abs(1-2*(num_errors/(Quant_Range*min_l))))
                if (k == 8) & (ind == 0):
                    plot_histogram.create_histogram(SDR1_2, 4, axis4, f'{k}-bitUSclip', 'black')
                    plot_histogram.create_histogram(floor_diff, 4, ax, f'{k}-bitUSclip', 'black')
                    print("floor diff", floor_diff)
                    plot_histogram.create_histogram(error_dist, 4, axis5, f'{k}-bitUSclip', 'black')
            label = f'{k}'
...
confidence_interval.plot_confidence_interval(np.array(err_values_gray).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3,  "gray no filters", 'blue', '')
...
confidence_interval.plot_confidence_interval(np.array(err_values_unitstep_gray).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3,  "gray unit step", 'magenta', 'o')
...
confidence_interval.plot_confidence_interval(np.array(CR_rate_clip).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3,  "CRrate Clip", 'red', mark)
confidence_interval.plot_confidence_interval(np.array(entropy_clip).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3,  "Capacity Clip", 'red', mark_cap)

confidence_interval.plot_confidence_interval(np.array(CR_rate).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "CRrate", 'blue', mark)
confidence_interval.plot_confidence_interval(np.array(entropy).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "Capacity", 'blue', mark_cap)

confidence_interval.plot_confidence_interval(np.array(CR_rate_US).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "CRrateUS", 'magenta', mark)
confidence_interval.plot_confidence_interval(np.array(entropy_US).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "CapacityUS", 'magenta', mark_cap)

confidence_interval.plot_confidence_interval(np.array(CR_rate_USclip).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "CRrateUSclip", 'black', mark)
confidence_interval.plot_confidence_interval(np.array(entropy_USclip).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "CapacityUSclip", 'black', mark_cap)


plt3.xlabel('Order of Quantization')
plt3.ylabel('Bits per observation')
plt3.show()
