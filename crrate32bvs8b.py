...
for ind in range(0, min_length, min_l):
    ...
    for mode in range(9):
        ...
        # Sav Gol
            ...
            print("Unfiltered 8 bit Obsd.", RSSI_SDR1_8.raw_samples[ind:ind + min_l])

            SDR1_1_norm_float = noise_removal.savgold_filter_ali(RSSI_SDR1_8.raw_samples[ind:ind + min_l],
                                                       9, 5)  # 8-bit Savgol
            SDR2_1_norm_float = noise_removal.savgold_filter_ali(RSSI_SDR2_8.raw_samples[ind:ind + min_l], 9, 5)


            SDR1_1_norm = [round(x) for x in SDR1_1_norm_float]
            SDR2_1_norm = [round(x) for x in SDR2_1_norm_float]

            print("filtered 8 bit Obsd.", SDR1_1_norm)

        elif mode == 3:
            SDR1_1_norm = noise_removal.savgold_filter(RSSI_SDR1_32.raw_samples[ind:ind + min_l], win - 1) #32-bit Savgol
            SDR2_1_norm = noise_removal.savgold_filter(RSSI_SDR2_32.raw_samples[ind:ind + min_l], win - 1)

        #DCT
        elif mode == 4:
            SDR1_1_norm_float = dct.adaptive_dct_filter_8bit(RSSI_SDR1_8.raw_samples[ind:ind + min_l]) #8-bit DCT
            SDR2_1_norm_float = dct.adaptive_dct_filter_8bit(RSSI_SDR2_8.raw_samples[ind:ind + min_l])
        ...
        for k in range(2, maxQuantrange + 1):
            ...
            if mode == 0:
                ...
                entropy = calculate_entropy.calculate_entropy(SDR1_2)
                No_Filter_32.entropy.append(entropy)
                ...
                entropy = calculate_entropy.calculate_entropy(SDR1_2)
                Savgol_32.entropy.append(entropy)
                Savgol_32.CR_rate.append((entropy)*abs(1-2*(num_errors/(Quant_Range*min_l))))


            elif mode == 4:
                sample_entropy = calculate_entropy.calculate_entropy(list_of_int_SDR1[ind:ind + min_l])
                #DCT_8.entropy.append(8)
                DCT_8.entropy.append(sample_entropy)
                entropy = calculate_entropy.calculate_entropy(SDR1_2)
                ...
                DCT_32.entropy.append(sample_entropy)
                entropy = calculate_entropy.calculate_entropy(SDR1_2)
...
confidence_interval.plot_confidence_interval(np.array(DCT_32.entropy).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "CR Cap. 32-bit", 'magenta', mark_cap)
...
confidence_interval.plot_confidence_interval(np.array(DCT_32.CR_rate).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "DCT 32-bit", 'cyan', mark)
confidence_interval.plot_confidence_interval(np.array(DCT_8.CR_rate).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "DCT 8-bit", 'blue', mark)
confidence_interval.plot_confidence_interval(np.array(Savgol_32.CR_rate).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "Sav. Gol. 32-bit", 'brown', mark)
confidence_interval.plot_confidence_interval(np.array(Savgol_8.CR_rate).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "Sav. Gol. 8-bit", 'black', mark)
confidence_interval.plot_confidence_interval(np.array(No_Filter_32.CR_rate).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "NF 32-bit", 'red', mark)
confidence_interval.plot_confidence_interval(np.array(No_Filter_8.CR_rate).reshape(num_columns, num_rows).transpose(), np.array(quan_size), labelarray, axis3, "NF 8-bit", 'yellow', mark)
