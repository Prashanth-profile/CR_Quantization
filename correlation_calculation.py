...
def complete_correlation(min_length, data1, data2):
    number_of_samples = range(min_length-1)
    corr_coeff = []
    for i in range(1, len(number_of_samples)+1):
        #print("data is", data1[:i], data2[:i])
        #corr_coefficient, _ = spearmanr(data1[0:i], data2[0:i])
        corr_coefficient = np.corrcoef(data1[:i], data2[:i])
        corr_coeff.append(abs(corr_coefficient[0, 1]))
        #corr_coeff[i - 1] = abs(corr_coefficient)
