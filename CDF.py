...
for mode in range(0, 6):
    ...
    for ind in range(0, min_length, min_l):
        ...
        if mode==2 or mode==3:
            SDR1_1_norm = dct.adaptive_dct_filter_window(list_of_floats_SDR1[ind:ind + min_l], 2) #32bit DCT
            SDR2_1_norm = dct.adaptive_dct_filter_window(list_of_floats_SDR2[ind:ind + min_l], 2)
    ...
    if mode==0 or mode==1:
        ...
        plt.plot(x, y, label="DCT $I = n/2$, $c = {}$".format(maxQuantrange))
    elif mode==2 or mode==3:
        print("x,y DCT $I = 2$", x, y)
        plt.plot(x, y, label="DCT $I = 2$, $c = {}$".format(maxQuantrange))
    elif mode==4 or mode==5:
        print("x,y SG", x, y)
        plt.plot(x, y, label="Sav. Gol. $c = {}$".format(maxQuantrange))
